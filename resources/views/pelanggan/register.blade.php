          ...
          <form action="{{url('register')}}" method="POST">
            ...
            <div class="row">
              <div class="col-12">
                ...
                <input type="text" name="npwp" id="npwp" class="input-design" required value="{{old('npwp')}}">
              ...
              <div class="col-12 col-lg-6">
                ...
                <input type="text" name="email" id="email" class="input-design" required value="{{old('email')}}">
                @foreach ($errors->get('email') as $message)
                <small id="emailHelp" class="form-text text-danger">{{$message}}</small>
                @endforeach
                <label for="password" class="d-block">Password <span>*</span></label>
                <input type="password" name="password" id="password" class="input-design" required value="{{old('password')}}">
              ...
              <div class="col-12 col-lg-6">
                ...
                <input type="text" name="nama" id="nama" class="input-design" required value="{{old('nama')}}">
                @foreach ($errors->get('nama') as $message)
                <small id="emailHelp" class="form-text text-danger">{{$message}}</small>
                @endforeach
                <label for="no_telp" class="d-block">No. Telp <span>*</span></label>
                <input type="telp" name="no_telp" id="no_telp" class="input-design" required value="{{old('no_telp')}}">
